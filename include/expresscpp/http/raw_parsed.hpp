...
namespace express {
	namespace http {
		...
		public:
			...
			constexpr version(value version) : version_(version) {	}

			inline operator std::string()
			{
				if (version_ == HTTP_0_9)
					return "HTTP/0.9";
		...
		public:
			...
			constexpr status(uint16_t status) : status_(static_cast<value>(status)) { }

			constexpr status(value status) : status_(status) { }
